...
#ifndef __CutFlowJetLine__
#define __CutFlowJetLine__

#include "TNamed.h"


class CutFlowJetLine : public TNamed {
public:
  CutFlowJetLine(void);
  CutFlowJetLine(const TString &name, const TString &title="");
  ...
  void PassCut(const Double_t& weight, Int_t NJets);// { _exposedCount++; _exposedYield+=weight; _passCount++; _passYield+=weight; };
  void FailCut(const Double_t& weight); //{ _exposedCount++; _exposedYield+=weight; };
...
private:
  ...
  Int_t    _passCount[10];  // number of events passing this cut
  Double_t _passYield[10];  // sum of weights for events passing this cut

  ClassDef (CutFlowJetLine, 0);
};
