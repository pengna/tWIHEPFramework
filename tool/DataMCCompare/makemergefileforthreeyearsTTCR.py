...
mergeyears = file(Analyzerpath+"mergeyearsTTCR.sh","w")
mergechannel = file(Analyzerpath+"mergechannelTTCR.sh","w")
mergeAll = file(Analyzerpath+"mergecombineTTCR.sh","w")
for dirdetail in dirdetails:
	for sample in samples:
		rootfilename = "output_"+sample+".root " 
		for syst in systs:
			Muon2016dirpath = Analyzerpath+dirdetail+"MuonTTCR2016/"+syst+"/"
			Muon2017dirpath = Analyzerpath+dirdetail+"MuonTTCR2017/"+syst+"/"
			Muon2018dirpath = Analyzerpath+dirdetail+"MuonTTCR2018/"+syst+"/"
			Electron2016dirpath = Analyzerpath+dirdetail+"ElectronTTCR2016/"+syst+"/"
                	Electron2017dirpath = Analyzerpath+dirdetail+"ElectronTTCR2017/"+syst+"/"
                	Electron2018dirpath = Analyzerpath+dirdetail+"ElectronTTCR2018/"+syst+"/"

			MuonFullRunIIdirpath = Analyzerpath+dirdetail+"MuonTTCRFullRunII/"+syst+"/"
			ElectronFullRunIIdirpath = Analyzerpath+dirdetail+"ElectronTTCRFullRunII/"+syst+"/"
			Combine2016dirpath = Analyzerpath+dirdetail+"CombineTTCR2016/"+syst+"/"
			Combine2017dirpath = Analyzerpath+dirdetail+"CombineTTCR2017/"+syst+"/"
			Combine2018dirpath = Analyzerpath+dirdetail+"CombineTTCR2018/"+syst+"/"
			CombineFullRunIIdirpath = Analyzerpath+dirdetail+"CombineTTCRFullRunII/"+syst+"/"
