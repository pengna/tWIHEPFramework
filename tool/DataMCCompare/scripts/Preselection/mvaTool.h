...
class mvaTool {
 ...
 private:
  ...
  void loopInSample(TString dirName, TString sampleName, float* treevars, bool isData, bool doMVA = true, int Era =2016 , TString channel ="Muon");
  void createHists(TString sampleName);
  void fillHists(TString sysName,TString sampleName, float* treevars, double mvaValue, double mvawJets, double theweight, float met, float mtw,float DeltaRPhilv,float DeltaRPhivboostedjet, float DeltaRPhitW, float HT,float ST,float WOverTop_pT,int theChannel);
