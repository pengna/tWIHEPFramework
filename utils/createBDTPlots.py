...
perMCSFs["qcd"] = 0.795672391198
perMCSFs["ttbar"] = 2.

perMCSFs["ttbar"] = 2.*0.906745514628
perMCSFs["wPlusJets"] = 3.54872256164
...
for sample in samples:
    inFiles[sample] = TFile(inDir+"output_"+sample+".root","READ")
    
for sample in runRanges:
    inFiles["data"+sample] = TFile(inDir+"output_SingMu"+sample+".root","READ")
    inFiles["qcd"+sample] = TFile(inDir[:-1]+"QCD/output_SingMu"+sample+".root","READ")
...
for obj in inFiles["tW_top_nfh"].GetListOfKeys():
    ...
    if not tempThing.ClassName().find("TH1") == -1 : 
        if "up" in tempThing.GetName() and not tempThing.GetName().split("_")[-2] in systPaths:
            systPaths.append(tempThing.GetName().split("_")[-2])
        if (not "up" in tempThing.GetName() and not "down" in tempThing.GetName() and not tempThing.GetName().split("tW_top_nfh")[0]+"sample"+(tempThing.GetName().split("tW_top_nfh")[-1]) in plotPaths):
            plotPaths.append(tempThing.GetName().split("tW_top_nfh")[0]+"sample"+(tempThing.GetName().split("tW_top_nfh")[-1]))
...
#    if not plotName.GetName().split("_")[0] in plotNames: plotNames.append( plotName.GetName().split("_")[0] )
    ...
    for sample in samples:
        if histoGramPerSample[sample] in histMap.keys():
            histMap[histoGramPerSample[sample]].Add(inFiles[sample].Get(plotName.split("sample")[0]+sample+plotName.split("sample")[1]))
        else:
            histMap[histoGramPerSample[sample]] = inFiles[sample].Get(plotName.split("sample")[0]+sample+plotName.split("sample")[1])
            histMap[histoGramPerSample[sample]].SetName(plotName.split("sample")[0]+histoGramPerSample[sample]+plotName.split("sample")[1])

    
    #Here we will sum up the QCD histograms.
    for run in runRanges:
        if "qcd" not in histMap.keys():
            histMap["qcd"] = inFiles["qcd"+run].Get(plotName.split("sample")[0]+"SingMu"+run+plotName.split("sample")[1])
            histMap["qcd"].SetName(plotName.split("sample")[0]+"qcd"+plotName.split("sample")[1])
            histMap["data"] = inFiles["data"+run].Get(plotName.split("sample")[0]+"SingMu"+run+plotName.split("sample")[1])
            histMap["data"].SetName(plotName.split("sample")[0]+"data"+plotName.split("sample")[1])
            
        else:
            histMap["qcd"].Add(inFiles["qcd"+run].Get(plotName.split("sample")[0]+"SingMu"+run+plotName.split("sample")[1]))
            histMap["data"].Add(inFiles["data"+run].Get(plotName.split("sample")[0]+"SingMu"+run+plotName.split("sample")[1]))
