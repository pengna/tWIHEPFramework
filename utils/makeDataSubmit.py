...
def prepareCshJob(sample,shFile,frameworkDir,workpath,samplePost=""):
	...
	print >> subFile, frameworkDir+"bin/Wt/Wt_generic.x -config "+frameworkDir+"SingleTop.Wt.LP.mm1+j.muonMSSmeardown.config -inlist "+frameworkDir+"config/files/"+fileListDirectory+sample+samplePost+".list -hfile "+workpath+"/"+sample+"/hists/"+sample+samplePost+"hists.root -skimfile "+workpath+"/"+sample+"/skims/"+sample+samplePost+"Skim.root -BkgdTreeName DiElectronPreTagTree  -UseTotalEvtFromFile -SelectTrigger Muon"
