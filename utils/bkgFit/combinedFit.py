...
#        canvy =  inFile.Get(prim.GetName())
    ...
    dataH,qcdH,ttbarH,wJetsH,mcH = 0,0,0,0,0
    ...
    for hist in stack.GetHists():
        ...
        elif "ttbar" in hist.GetName(): ttbarH = hist
        else:
            if mcH == 0: mcH = hist
            else: mcH.Add(hist)

    return dataH,qcdH,ttbarH,wJetsH,mcH

def doFit(data,qcd,ttbar,wJets,mc):
    ...
    ttbarCoeff = RooRealVar("ttbarCoeff","ttbarCoeff",ttbar.Integral(),0.,5*ttbar.Integral())
    ...
    modelCoeff2 = RooRealVar("modelCoeff2","modelCoeff2",0.5,0.,1.)

    pdf_sum = RooAddPdf("pdf_sum","pdf_sum",RooArgList(mc_template_pdf,ttbar_template_pdf,wJ_template_pdf,qcd_template_pdf),RooArgList(mcCoeff,ttbarCoeff,wJCoeff,qcdCoeff))

    print "Does the first one"
    pdf_sum_reduced = RooAddPdf("pdf_sum","pdf_sum",RooArgList(ttbar_template_pdf,wJ_template_pdf,qcd_template_pdf),RooArgList(modelCoeff,modelCoeff2))

    print "Does the second one"
    ...
    print "Nevents total MC	  = ", qcd.Integral() + ttbar.Integral() + wJets.Integral() + mc.Integral()
    ...
    print "Total MC after fit  = ", wJCoeff.getVal() + qcdCoeff.getVal() + mcCoeff.getVal() + ttbarCoeff.getVal()
    print "Total Data	       = ", data.Integral()
    print "Model Coeff         = ", modelCoeff.getVal()
    print "Model Coeff2        = ", modelCoeff2.getVal()
    ...
    pdf_sum.plotOn(frame,RooFit.Components("ttbar_template_pdf"),RooFit.VisualizeError(fitResult),RooFit.FillColor(kTeal),RooFit.LineWidth(2))
...
def main():
    print "Entering main"
    data, qcd, ttbar, wJets, mc = getHists(sys.argv[1])

#    mc.Scale(-1.)
#    data.Add(mc)

    doFit(data,qcd,ttbar,wJets,mc)
