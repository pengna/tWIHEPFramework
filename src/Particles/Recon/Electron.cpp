...
//Bool_t Electron::Fill(EventTree *evtr, Int_t iE, TString electronType, Bool_t isSimulation)
{
  ...
  Double_t elEsc         = evtr -> patElectron_Esc -> operator[](iE);
...
//  Int_t trigger        = evtr -> HLT_Ele23_WPLoose_Gsf;
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(!passHEEPId())				     passHEEP  = kFALSE;




   if (TMath::Abs(scEta()) <= 1.479){ //Electron in barrel
    if (sigmaEtaEta() >= 0.0112 ||
        TMath::Abs(dEtaInSeed()) >= 0.00377 || 
        TMath::Abs(dPhiIn()) >= 0.0884 || 
   //   hOverE() >= 0.079 ||
      hOverE() >= 0.05+1.16/elEsc+0.0324*rho/elEsc ||
       ooEmooP() >= 0.193 ||
       missingHits() > 1 ||
        !passConversionVeto()
        )
      passIDLoosenoIso = kFALSE;
   // if (relIsoPFRhoEA() > 0.046){
    if (relIsoPFRhoEA() > 0.112+0.506/elPt){
      passLooseIso = kFALSE;
    }
  }
  else { // Electron in endcap
    if (sigmaEtaEta() >= 0.0425 ||
        TMath::Abs(dEtaInSeed()) >= 0.00674 || 
        TMath::Abs(dPhiIn()) >= 0.169 || 
   //   hOverE() >= 0.0637 ||
       hOverE() >= 0.0441+2.54/elEsc+0.183*rho/elEsc||
        ooEmooP() >= 0.111 ||
        missingHits() > 1 ||
        !passConversionVeto()
        )
      passIDLoosenoIso = kFALSE;
   //   if (relIsoPFRhoEA() > 0.0759) {
   if (relIsoPFRhoEA() > 0.108+0.963/elPt) {
      passLooseIso = kFALSE;
    }
  }
   ...
   //   hOverE() >= 0.079 ||
      hOverE() >= 0.026+1.15/elEsc+0.0324*rho/elEsc ||
       ooEmooP() >= 0.159 ||
       missingHits() > 1 ||
        !passConversionVeto()
        )
   ...
   //   hOverE() >= 0.0637 ||
       hOverE() >= 0.0188+2.06/elEsc+0.183*rho/elEsc||
        ooEmooP() >= 0.0197 ||
        missingHits() > 1 ||
        !passConversionVeto()
        )
...
for (auto const & jet : selectedjets){
  ...
  if (_QCD_CR) {
	  if(!passEleclosetJetDeatR&&elrelpt>_relPtcut)passEleJet2Dcut =kFALSE;
  }
  else {
	  if(!passEleclosetJetDeatR&&elrelpt<_relPtcut)passEleJet2Dcut =kFALSE;
  }
