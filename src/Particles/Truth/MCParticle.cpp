...
const MCParticle MCParticle::GetGenPartcileNoFsr(const MCParticle Ptemp, std::vector<MCParticle>& MCParticles) const
{
//	cout<<"start pdg id = "<<Ptemp.PdgId()<< "Ptemp.numDaught = "<<Ptemp.numDaught()<<endl; 
  if (Ptemp.isJet() ||Ptemp.isMuon() ||Ptemp.isElectron() || Ptemp.isNeutrino()) return Ptemp;
  else{
	if(Ptemp.numDaught()>0){
	  for(int d_index=0; d_index< Ptemp.BdaughtIndices().size(); d_index++){	
			const MCParticle daughter =MCParticles.at(Ptemp.BdaughtIndices().at(d_index)); 
			if(Ptemp.PdgId()==daughter.PdgId())return GetGenPartcileNoFsr(daughter,MCParticles);
		}
}
}   
		return Ptemp;			
...
Bool_t MCParticle::isFromTop(const MCParticle Ptemp, std::vector<MCParticle>& MCParticles, int topid) const{
  for(int m_index=0; m_index< Ptemp.BmotherIndices().size(); m_index++){
    const MCParticle mother = MCParticles.at(Ptemp.BmotherIndices().at(m_index));
    int momid = mother.AbsPdgId();
...
Bool_t MCParticle::IsHadronicDecay(const MCParticle PTemp, std::vector<MCParticle>& MCParticles) const
{
	Bool_t decaytojet = true;
        const MCParticle Ptemp= GetGenPartcileNoFsr(PTemp,MCParticles);
     //   if(Ptemp.Status()==2){
                for(int d_index=0; d_index< Ptemp.BdaughtIndices().size(); d_index++){
                        const MCParticle daught = MCParticles.at(Ptemp.BdaughtIndices().at(d_index));
                        int daughtid = daught.AbsPdgId();
			if(!daught.isJet())decaytojet= false;
		//	cout<<" final jet pdg id = "<<daught.PdgId()<<endl;
	}
                return decaytojet;
