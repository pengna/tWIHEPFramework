...
#include "SingleTopRootAnalysis/Base/Dictionary/Utilities.hpp"
//#include "src/Base/Dictionary/Utilities.cpp"
#include <iostream>
#include <iomanip>
#include <fstream>
#include <sstream>
...
JetCorrectorParameters::Definitions::Definitions(const std::string& fLine)
{
  ...
  if (!tokens.empty())
      ...
      if (tokens.size() < 6) 
        {
          ...
          std::cout<<"(line "<<fLine<<"): less than 6 expected tokens:"<<tokens.size();
	...
	mIsResponse = false;
	  ...
	  std::cout <<"unknown option ("<<ss<<")" << std::endl << std::flush;
          handleError("JetCorrectorParameters::Definitions",sserr.str());
        }
