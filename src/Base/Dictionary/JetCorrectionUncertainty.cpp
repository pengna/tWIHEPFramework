...
#include "SingleTopRootAnalysis/Base/Dictionary/SimpleJetCorrectionUncertainty.hpp"
      ...
      if (fNames[i] == "JetEta")
        {
          if (!mIsJetEtaset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" jet eta is not set";
      ...
      else if (fNames[i] == "JetPt")
        {
          if (!mIsJetPtset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" jet pt is not set";  
      ...
      else if (fNames[i] == "JetPhi")
        {
          if (!mIsJetPhiset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" jet phi is not set";  
          result.push_back(mJetPt);
        }
      else if (fNames[i] == "JetE")
        {
          if (!mIsJetEset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" jet energy is not set";
	   ...
	   std::cerr << " jet emf is not set";
      ...
      else if (fNames[i] == "LepPx")
        {
          if (!mIsLepPxset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" lepton px is not set";  
      ...
      else if (fNames[i] == "LepPy")
        {
          if (!mIsLepPyset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" lepton py is not set";  
      ...
      else if (fNames[i] == "LepPz")
        {
          if (!mIsLepPzset)
            //throw cms::Exception(
	   std::cerr << "JetCorrectionUncertainty::"<<" lepton pz is not set";  
          result.push_back(mLepPz);
        }
      else
        //throw cms::Exception(
	std::cerr << "JetCorrectionUncertainty::"<<" unknown parameter "<<fNames[i];
    }     
  return result;      
}
//------------------------------------------------------------------------ 
...
float JetCorrectionUncertainty::getPtRel()
{
  ...
  if (!(lj2 > 0))
    //throw cms::Exception(
   std::cerr << "JetCorrectionUncertainty"<<" not positive lepton-jet momentum: "<<lj2;
