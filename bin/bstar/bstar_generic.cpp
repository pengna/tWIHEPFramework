...
#endif
{
 ...
 // Check command line for Fast Running
  ...
  TString MuonTypeToSelect = "Tight"; //This variable is altered to unisolated when running QCD estimation.
  TString ElectronTypeToSelect = "Tight"; //This variable is altered to unisolated when running QCD estimation.
  TString leptonTypeToSelect = "Tight"; //This variable is altered to unisolated when running QCD estimation.
  TString leptonTypeToVeto = "Veto"; //This variable is altered to unisolated when running QCD estimation.
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!strcmp(argv[i], "-TopPtreweight")){
      useTopPtreweight= kTRUE;
      cout << "Driver: Using TopPtreweight" << endl;
    }
	...
	return 1;
      ...
      if(!strcmp(argv[i+1],"bstarMuon")) {whichtrig = 2; MuonTypeToSelect=leptonTypeToSelect; ElectronTypeToSelect=leptonTypeToVeto; }
      if(!strcmp(argv[i+1],"bstarElectron")) {whichtrig = 3;MuonTypeToSelect=leptonTypeToVeto; ElectronTypeToSelect=leptonTypeToSelect;}
      if( whichtrig == -1){
...
//  mystudy.AddCut(new CutJetN(particlesObj,nJets));
 ...
 //mystudy.AddCut(new CutElectronN(particlesObj, "Veto")); //require that lepton to be isolated, central, high pt
  ...
  mystudy.AddCut(new HistogrammingMuon(particlesObj,MuonTypeToSelect));  // make the muon plots, hopefully.

  mystudy.AddCut(new HistogrammingNPvtx(particlesObj));

  mystudy.AddVars(new bstarVars(true));
